...
// Xử lý ảnh sản phẩm
$main_image = !empty($product['image_url']) ? $product['image_url'] : 'images/default-product.jpg';
...
if (isset($_GET['action']) && $_GET['action'] == 'delete_review' && isset($_GET['review_id'])) {
    ...
    // Kiểm tra xem review này có thuộc về người dùng hiện tại không
    $check_owner = "SELECT 1 FROM product_reviews WHERE id = ? AND user_id = ?";
    ...
    if ($is_owner) {
        ...
        header("Location: product_detail.php?id=$product_id&success=3");
        exit();
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_review']) && $has_purchased) {
    ...
    $check_review = "SELECT 1 FROM product_reviews WHERE product_id = ? AND user_id = ?";
    ...
    if ($already_reviewed) {
        ...
        $sql = "UPDATE product_reviews SET rating = ?, comment = ?, created_at = NOW() 
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_question'])) {
    ...
    header("Location: product_detail.php?id=$product_id&success=2");
    exit();
}
...
<body class="bg-gray-100">
    ...
    <main class="container mx-auto px-4 py-8">
        <?php if (isset($_GET['success'])): ?>
            <div class="mb-4 p-3 rounded-md <?php 
                echo $_GET['success'] == 1 ? 'bg-green-100 text-green-800' : 
                ($_GET['success'] == 2 ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'); 
            ?>">
                <?php 
                echo $_GET['success'] == 1 ? 'Đã gửi đánh giá thành công!' : 
                ($_GET['success'] == 2 ? 'Đã gửi câu hỏi thành công!' : 'Đã xóa đánh giá thành công!'); 
                ?>
            </div>
        <?php endif; ?>
                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-3">
                        ...
                        <li>
                            <div class="flex items-center">
                                ...
                                <a href="giay.php" class="text-blue-600 hover:text-blue-800">Sản phẩm</a>
                            </div>
                        </li>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        <div class="mb-4 bg-gray-100 rounded-lg overflow-hidden">
                            <img id="main-image" src="<?php echo htmlspecialchars($main_image); ?>" 
                                    ...
                                    ?>
                                        <label class="inline-flex items-center">
                                            <input type="radio" name="size" value="<?php echo $size; ?>" 
                                                   class="hidden peer" <?php echo $size === '39' ? 'checked' : ''; ?>>
                        ...
                        <div>
                            ...
                            <div class="bg-gray-50 p-4 rounded-lg mb-6">
                                ...
                                <form method="POST">
                                                ...
                                                <?php for ($i = 1; $i <= 5; $i++): ?>
                                                    <label>
                                                        <input type="radio" name="rating" value="<?php echo $i; ?>" 
                                                               <?php echo ($user_review && $i == $user_review['rating']) ? 'checked' : ''; ?> 
                                                               required class="hidden">
                                                        <i class="fas fa-star cursor-pointer text-2xl mx-1 star <?php 
                                                            echo ($user_review && $i <= $user_review['rating']) ? 'active text-yellow-400' : 'text-gray-300';
                                                        ?>" data-value="<?php echo $i; ?>"></i>
                                                    </label>
                                                <?php endfor; ?>
                                            </div>
                                        </div>
                                        <textarea name="comment" rows="3" class="w-full border rounded p-2" placeholder="Nhận xét của bạn..." required><?php 
                                    ...
                                    <?php if ($user_review): ?>
                                        <a href="product_detail.php?id=<?php echo $product_id; ?>&action=delete_review&review_id=<?php echo $user_review['id']; ?>" 
                            ...
                            <?php elseif (isset($_SESSION['user_id'])): ?>
                                <div class="bg-blue-50 p-4 rounded-lg mb-6 border border-blue-200">
                                    <p class="text-blue-800">Bạn cần mua sản phẩm này trước khi có thể đánh giá.</p>
                                    <a href="giay.php" class="inline-block mt-2 text-blue-600 hover:underline">Tiếp tục mua sắm</a>
                                </div>
                            <?php else: ?>
                                <div class="bg-gray-100 p-4 rounded-lg mb-6 border border-gray-200">
                                    <p class="text-gray-700">Vui lòng <a href="login.php" class="text-blue-600 hover:underline">đăng nhập</a> và mua sản phẩm để đánh giá.</p>
                                </div>
                            <?php endif; ?>
                                ...
                                <div class="border-b pb-4 mb-4">
                                    ...
                                    <div class="bg-blue-50 p-3 rounded mt-2">
                                        ...
                                        <?php if ($question['answered_at']): ?>
                                            <p class="text-xs text-blue-600 mt-1">Trả lời ngày <?php echo date('d/m/Y', strtotime($question['answered_at'])); ?></p>
                                        <?php endif; ?>
                                    </div>
                                    <?php endif; ?>
                                </div>
        ...
        <div class="container mx-auto px-4">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-8">
                <div>
                    ...
                    <p class="mb-4">Chuyên cung cấp giày thể thao chất lượng cao với giá cả hợp lý.</p>
    ...
    <script>
        ...
        document.getElementById('add-to-cart').addEventListener('click', function() {
            ...
            const image = "<?php echo addslashes($main_image); ?>";
        ...
        // Khởi tạo hiển thị sao đánh giá của người dùng (nếu có)
        <?php if ($user_review): ?>
        highlightStars(<?php echo $user_review['rating']; ?>);
        <?php endif; ?>
    </script>
</body>
</html>
